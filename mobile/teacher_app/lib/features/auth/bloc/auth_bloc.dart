...
class AuthLoginRequested extends AuthEvent {
  final String identifier, password, tenantSlug;
  ...
  AuthBloc({required this.dio, required this.storage}) : super(AuthInitial()) {
    on<AuthLoginRequested>((event, emit) async {
      ...
      try {
        ...
        final res = await dio.post('/auth/login', data: {'identifier': event.identifier, 'password': event.password});
        await storage.write(key: 'access_token', value: res.data['data']['accessToken']);
        await storage.write(key: 'refresh_token', value: res.data['data']['refreshToken']);
        emit(AuthAuthenticated(res.data['data']['user']));
      } catch (e) { emit(AuthError(e.toString())); }
    });
    on<AuthLogoutRequested>((event, emit) async { await storage.deleteAll(); emit(AuthUnauthenticated()); });
  }
}
